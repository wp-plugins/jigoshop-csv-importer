...
$data    = array();
$titles    = array();
if ( isset( $_REQUEST['wc_import_csv'] ) && isset( $_FILES['upload_file'] ) ) {
...
} elseif ( isset( $_REQUEST['wc_import_products_from_csv'] ) && isset( $_SESSION['wc_csv_titles'] ) ) {
...
<form method="post" enctype="multipart/form-data">
    ...
    <td>
        ...
        <input type="submit" name="wc_import_taxonomies" value="<?php _e( 'Import taxonomies', 'tcp' );?>" class="button-secondary"/>
    </td>
    </tr>
    <tr valign="top">
    <th scope="row">
...
if (preg_match('/pa_/', $taxmy) == 0)  {?>
    ...
    <span class="submit"><input type="submit" name="wc_import_csv" id="wc_import_csv" value="<?php _e( 'Import', 'wc_csvl' );?>" style="button-secondary" /></span>
    <span><?php _e( 'This action helps you to test if the file is correct. Only 4 rows will be displayed.', 'wc_csvl' );?></span>
</form>
<?php if ( is_array( $data ) && count( $data ) > 0 ) : ?>
<p><?php _e( 'These lines are the four first products Imported from the CSV file. If you think they are correct continue with the process.', 'wc_csvl' );?></p>
...
<thead>
    <tr scope="col" class="manage-column">
        ...
        <th><?php _e( 'Jigoshop columns', 'wc_csvl' );?></th>
    </tr>
</thead>
<tfoot>
    <tr scope="col" class="manage-column">
...
<span class="submit">
    <input type="submit" name="wc_import_products_from_csv" id="wc_import_products_from_csv" value="<?php _e( 'Upload', 'wc_csvl' );?>" class="button-primary" />
    <span><?php _e( 'This action will import the products in the eCommerce. Be patient.', 'wc_csvl' );?></span>
</span>
</form>
<?php endif;?>
</div>
